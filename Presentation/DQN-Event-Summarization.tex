...
\section{Experiments}
	\subsection{Simulation}
		...
		Studying in-sample behavior on a small-sample problem allows us to
		\begin{enumerate}
		...
		\begin{frame}
			...
			Currently running full model on 44 queries each with ~15,000 sentences
			\begin{center}
				\begin{itemize}
					\item<1-> Challenges arise in slow compute time because of the forward step has to sequentially update the current summary
					\item<1-> Long training time required in short simulation showed that to learn effective policy we have to explore a large state-space and 
				\end{itemize}
			\end{center}
		\end{frame}

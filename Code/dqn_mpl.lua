...
enc:add(nn.Linear(hiddenSize, 2))
enc:add(nn.LogSoftMax())
-- enc = nn.Sequencer(enc)
--- Add mlp

print(enc:forward(x:t()))

cr = nn.ClassNLLCriterion()
seqC = nn.SequencerCriterion(cr)
-- criterion = nn.SequencerCriterion(nn.MaskZeroCriterion(cr, 1))
err = 0

out = enc:forward(x:t())
err = err + seqC:forward(out, y)
gradOut = seqC:backward(out, y)
enc:backward(x, gradOut)

--We update params at the end of each batch
enc:updateParameters(0.05)

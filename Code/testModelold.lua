...
model = buildModel(nnmod, vocabSize, embeddingSize, metric, use_cuda)
...
for epoch=0, nepochs do
    for query_id=1, #inputs do
        ...
        --- Running backprop
        -- inputs = {fullmemory[1], fullmemory[3]}
        -- local rewards = fullmemory[2]
        -- local dataloader = dl.TensorLoader(inputs, rewards)
        -- for k, xin, reward in dataloader:sampleiter(batch_size, memsize) do
        --     xinput = xin[1]
        --     actions_in = xin[2]
        ...
        --     end
         --- optim.rmsprop returns \theta, f(\theta):= loss function
         -- _, loss  = optim.rmsprop(feval, params, optimParams)   

...
\pgfsetlayers{background,main,foreground}
    ...
    \tikzstyle{wa} = [sensor, text width=8em, fill=red!20,  minimum height=2em, rounded corners, drop shadow]
    \def\edgedist{2.5}
    \begin{tikzpicture}
        \node (wa) [wa] {\textbf{Linear} };
        \path (wa.west)+(-2.0 ,  2.0) node (asr1)[sensor] {$LSTM$};
        \path (wa.west)+(-2.0 ,  0.0) node (asr2)[sensor] {$LSTM$};
        \path (wa.west)+(-2.0 , -2.0) node (asr3)[sensor] {$LSTM$};
        \path (wa.east)+(2.0,  1) node (vote) [term] {\textbf{ReLU}};
        \path (wa.east)+(2.0, -1) node (vote2) [term] {\textbf{ReLU}};
        \path (wa.east)+(4.0,  0) node (output) [term] {\textbf{E[Rouge]}} ;
     ...
     \begin{pgfonlayer}{background}
            ...
            \path (asr3.west  |- vote.east)+(+13.0, -2.5) node (d) {};
            \path (asr3.west  |- vote2.east)+(+13.0, -2.5) node (d) {};
...
\FOR{$epoch=1, N\ $}
	\FOR{$q \in \mathcal{Q}$}
		...
		\ENDFOR
	\ENDFOR
		\STATE Sample $J$ transitions from $\Gamma$
		\FOR{ $j=1,...,J$}
		\STATE \[\textrm{Set } y_j= 
					\begin{cases}

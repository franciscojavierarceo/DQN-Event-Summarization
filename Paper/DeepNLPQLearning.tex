...
\FOR{$epoch=1, N\ $}
	\FOR{$q \in \mathcal{Q}$}
		...
		\FOR{$x_t \in X_q $}
			\STATE $s_t \leftarrow  s(x_t, \tilde{y}_t, q)$
			\STATE  \textrm{Compute} $Q(s_t)$,  $ \forall a_t \in \mathcal{A}(s_t)$
			...
			\STATE Store transition $\Gamma \leftarrow \Gamma \cup \{ [s_t, a, r_t, s_{t+1}]\}$
	\ENDFOR
	\ENDFOR
		...
		\STATE 	\[ 		\textrm{Set } y_j= 
					...
					\end{cases} 
					\]
        		\STATE Perform gradient step on $(y_j - Q(s_j, a_j; \theta))^2$
\ENDFOR
  \end{algorithmic}

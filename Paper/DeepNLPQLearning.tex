...
\end{equation}
that learns the parameters using stochastic gradient descent with RMSprop \cite{hinton2012lecture} on sampled batches of the stored transitions.
...
\pgfsetlayers{background,main,foreground}
    ...
    \begin{tikzpicture}
     ...
     \begin{pgfonlayer}{background}[transparency group,opacity=.5]
        ...
        \path (wa.west) + (-2.90, -3.2)  node (vote.east) {Input}; %{\textbf{Input State}};
        \path (wa.west) + (3.5,    -3.2)  node (vote.east) {State-Action Representation }; %{\textbf{Summary-Action Representation} };
        \path (wa.west) + (9.30,  -3.2)  node (vote.east) {Output}; %{\textbf{Output State}};
...
\begin{tabular}{ l | c | c  }
	...
	Metric  & Model  & Performance\\ \hline \hline
  	ROUGUE-1F1 & DQN-LSTM  & TBD \\
  	ROUGUE-1F1 & DQN-BOW  & TBD \\
  	ROUGUE-1F1 & DQN-BiBOW  & TBD \\
  	ROUGUE-1F1 & Random  & TBD \\ \hline
  	ROUGUE-2F1 & DQN-LSTM  & TBD \\
  	ROUGUE-2F1 & DQN-BOW  & TBD \\
  	ROUGUE-2F1 & DQN-BiBOW  & TBD \\
  	ROUGUE-2F1 & Random  & TBD \\ \hline
	\hline
\end{tabular}
\end{center}

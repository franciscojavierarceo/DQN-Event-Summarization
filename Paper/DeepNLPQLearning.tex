...
\pgfsetlayers{background,main,foreground}
    ...
    \begin{tikzpicture}
     ...
     \begin{pgfonlayer}{background}[transparency group,opacity=.5]
        ...
        \path (wa.west) + (-2.90, -3.2)  node (vote.east) {\textbf{Input State}};
        \path (wa.west) + (3.5,    -3.2)  node (vote.east) {\textbf{Summary-Action Representation} };
        \path (wa.west) + (9.30,  -3.2)  node (vote.east) {\textbf{Output State}};
        \path (wa.east) + (0.5, -4.2)  node (vote.east) {Figure 1: DQN-LSTM for Event Summarization};
\end{tikzpicture}

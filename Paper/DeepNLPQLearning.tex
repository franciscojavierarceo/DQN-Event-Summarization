...
\FOR{$i \in \{1,.., N\} $}
	\FOR{$q \in \mathcal{Q}$}
		...
		\FOR{$x_t \in X_q  $}
			\STATE  \textrm{Compute} $Q(s(x_t, \tilde{y}_t, q))$ for all $a_t \in \mathcal{A}(s_t)$
			\STATE Select $a^{*}_t =$ argmax $Q(s(x_t, \tilde{y}_t, q))$
			\IF{$a^{*}_t = select$} 
				\STATE $\tilde{y}_{t+1} = \tilde{y} \cup  \{ x_t \} $
			\ENDIF
		\ENDFOR
		\STATE Execute action $a^{*}_t$ and observe reward $r_t$ and new state $s_{t+1}$
		\STATE Store transition $\Gamma \leftarrow \Gamma \cup \{ [s(x_t, \tilde{y}_t, q), a, r_t ]\}$
		\STATE Sample random minibatches of transitions $\gamma_j$ from $\Gamma$
		\STATE 	\[ 		\textrm{Set } y_j= 
					\begin{cases}
						r_j              								& \text{if $s_{j+1}$ is terminal } \\
					     	r_j + \gamma $max$_{a'}Q(\phi_{j+1}, a'; \theta) 	& \text{if $s_{j+1}$ is non-terminal } 
					\end{cases} 
					\]

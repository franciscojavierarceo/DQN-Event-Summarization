...
In \textbf{Figure 2a} we see the convergence of our model when trained on Precision. Notice the convergence of the model after 150 epochs. Similarly, we see convergence of Recall after roughly 100 epochs with small perturbations that would decline if further epochs were allotted. Lastly, We see that the F1 also converges. We validate our model against an Oracle that sequentially selects the candidate sentence if and only if the change in ROUGE-F1 is positive. We see that our model learns a policy on par with the Oracle after sufficient training time.

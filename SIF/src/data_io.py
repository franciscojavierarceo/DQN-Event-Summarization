...
def getWordmap(textfile):
    '''
    This reads the Glove dataset (or textfile) and
    returns the embedding dictionary and weights
    '''
    words = {}
    We = []
    f = open(textfile,'r')
    ...
    for i, line in enumerate(lines):
        line = line.split(" ")
        # start at index 1 because 0 is the word
        j = 1
        v = []
        while j < len(line):
...
def getWordWeight(weightfile, a=1e-3):
    ...
    for k, v in zip(word2weight.keys(), word2weight.values()):
        word2weight[k] = a / (a + (v/ N))
    return word2weight

def getWeight(words, word2weight):
    weight4ind = {}
    for word, ind in zip(words.keys(), words.values()):
